...
#具有类型判断、获取元信息等功能，具体建议查看下方提供的官方文档
...
# --------get和post装饰器----------
#用于增加__method__和__route__特殊属性，分别标记GET,POST方法和path
...
# 判断是否存在request参数，并且该参数要在其他普通的位置参数之后，
...
# 批量注册URL函数
'''
通常add_route()注册会调用很多次，而为了框架使用者更加方便，
可以编写了一个可以批量注册的函数，预期效果是：
只需向这个函数提供要批量注册函数的文件路径，新编写的函数就会筛选，
注册文件内所有符合注册条件的函数。
